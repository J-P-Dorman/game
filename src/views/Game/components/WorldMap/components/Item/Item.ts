...
import { createRenderAction, dispatchRender, renderNow } from "../../../GameLoops/RenderLoop/utils";
	...
	const renderActions = {
		...
		itemAnimateDefault: createRenderAction({
			...
			func: ({ action }) => {
				...
				const animationIndex = isAnimating ? frames.indexOf(state.currentSpriteKey) : 0;
